...
$(function() {
	$.extend({
		getBoardList:function(){
			...
			var searchUrl = "/ssrolcfront/boards/"+boardTable+"/"+categoryCode+"/"+pageNum;
		...
		viewArticle:function(boardTable,articleNo){
			var searchCategory = $("#categoryCode").val();
			
			location.href="/ssrolcfront/board/"+boardTable+"/"+articleNo+"?searchcategory="+searchCategory;
		}
	});
	...
	$(".faq_cate ul li").on("click","a",function(){
		...
		$(".faq_cate ul li a.on").removeClass("on");
		$(this).addClass("on");
		$("#pageNum").val("1");
		$.getBoardList();
	});
